...
function SpmCustomMetrics(tokens, processMetricsInterval, metricsApiEndpoint) {
    this.metricsUrl = (metricsApiEndpoint || 'http://spm-receiver.sematext.com/receiver/custom/receive.json?token=') + tokens.spm;
    this.tokens = tokens
    this.datapoints = []
    this.logs = []
    //this.events = []
...
SpmCustomMetrics.prototype.log = function (type, tags, messageText, fields, callback) {
    ...
    var options = {
        url: 'https://logsene-receiver.sematext.com/' + this.tokens.logsense + '/' + type,
        headers: {
            'User-Agent': 'node-spm',
...
SpmCustomMetrics.prototype.send = function () {
    ...
    request.post(options, function (err, res) {
        ...
        if (err)
            ...
            this.emit('send metrics', {source: 'log', err: err})
    })
}
